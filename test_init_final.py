...
class taskCog(commands.Cog): 
	...
	async def command_task_list(self, ctx : commands.Context):
		...
		if basicSetting[21] != "1":
			print("복구완료!")
			await dbLoad()
			await self.bot.get_channel(channel).send( '< 보탐봇 접속완료! (보이스 미사용) >', tts=False)
				...
				if basicSetting[21] == "1":
					try:
						...
						if self.bot.voice_clients[0].is_connected() :
							await self.bot.get_channel(channel).send( '< 보탐봇 접속완료! >', tts=False)
							self.checker = True
							print("복구완료!")
					except:
...
class mainCog(commands.Cog): 
	...
	async def race_(self, ctx):
		...
		if ctx.message.channel.id == basicSetting[7] or ctx.message.channel.id == basicSetting[19]:
			...
			if len(racing_member) <= 1:
				...
				return
			elif len(racing_member) >= 25:
				await ctx.send('레이스 인원이 24명 초과입니다.')
				return
			else :
	...
	async def command_rock_paper_scissors_game(self, ctx : commands.Context):
		...
		message_rock_paper_scissors : discord.message.Message = await ctx.send("안 내면 진다! 가위바위보")
